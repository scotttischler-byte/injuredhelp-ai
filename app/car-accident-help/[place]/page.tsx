...
import { getAllGeoHubSlugs, getGeoHubBySlug, stateHubSlug } from "@/lib/geo-routes";
...
export default async function GeoHubPage({ params }: Props) {
  ...
  if (hub.type === "city" && enrichedSlug) {
    const city = hub.profile.city;
    const state = hub.profile.state;
    graphs.push(
      breadcrumbJsonLd(origin, [
        { name: "Home", path: "/" },
        { name: state, path: `/${stateHubSlug(state)}` },
