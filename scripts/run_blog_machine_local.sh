...
INTERVAL="${BLOG_MACHINE_INTERVAL_SEC:-3600}"
BATCH="${BLOG_MACHINE_BATCH:-1}"
USE_AI="${BLOG_MACHINE_AI:-1}"

echo "WreckMatch blog machine — posting every ${INTERVAL}s (batch=${BATCH})"
...
while true; do
  ...
  if [ "$USE_AI" = "1" ]; then
    python scripts/wreckmatch_blog_autopilot.py --batch "$BATCH" --ai || true
  else
    python scripts/wreckmatch_blog_autopilot.py --batch "$BATCH" || true
  fi
