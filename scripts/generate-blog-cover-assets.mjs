#!/usr/bin/env node
/**
 * One unique cover per blog slug — real photo base + per-slug crop/grade/overlay.
 * Each slug gets a distinct crop and color grade so cards never look identical.
...
async function renderCover(slug) {
  ...
  const saturation = 0.88 + pick(buf, 20, 32) / 100;
  const hue = pick(buf, 24, 51) - 25;

  const meta = await sharp(basePath).metadata();
  const w = meta.width ?? 1600;
  const h = meta.height ?? 900;
  const cropW = Math.max(800, Math.floor(w * (0.45 + pick(buf, 28, 45) / 100)));
  const cropH = Math.max(450, Math.floor(h * (0.45 + pick(buf, 32, 45) / 100)));
...
console.log(`Wrote ${written} photo-based unique WebP covers to ${OUT_DIR}`);
